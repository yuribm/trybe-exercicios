...
function sendMarsTemperature(callbackTemperature, callbackError) {
  const chancesToSend = Math.floor((Math.random() * 100) + 1);
  const errorReason = 'Robot is busy';
  const temperature = getMarsTemperature();
  setTimeout(() => {
    if (chancesToSend <= 40) {
      callbackError(errorReason);
    } else {
      callbackTemperature(temperature);
    };
  }, messageDelay());
}
